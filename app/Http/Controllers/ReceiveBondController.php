...
class ReceiveBondController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = $data->where('bond_type_id',1)->where('branch_id', Auth::user()->branch_id)->orderBy('id', 'desc');


        $data_count = $data->count();

        return view('managements.accounts.receive_bond.index')
    ...
    public function show(Request $request)
    {
        $table_length = isset($_GET['table_length']) ? $_GET['table_length'] : 10;
        if ($table_length == '') $table_length = 10;
        $receive_bond = Bond::where('bond_type_id',1)->where('branch_id', Auth::user()->branch_id)->orderBy('id', 'desc')->paginate($table_length);

        return response()->json($receive_bond);
    }
    ...
    public function search(Request $request)
    {
        ...
        // if you use accessors like $receive_bond->actions, include them
    ...
    public function store(Request $request)
    {
        ...
        $receive_bond = new Bond();
        $receive_bond->branch_id = Auth::user()->branch_id;
        $receive_bond->bond_type_id = 1;
        $receive_bond->amount = $request->amount;
        $receive_bond->fund_id = $request->fund_id;
        $receive_bond->bank_id = $request->bank_id;
        $receive_bond->note = $request->note;
        $receive_bond->date = $request->date;
        $receive_bond->user_id = Auth::id();
        $receive_bond->post = $request->post == "on" ? 1 : 0;
        $receive_bond->save();
        $receive_bond->check_no = Auth::user()->branch_id . 1 . $receive_bond->id;
        $receive_bond->save();
        ...
        $receive_bond->journal_id = $journal->id;
        $receive_bond->save();
        ...
        JournalDetail::create([
            ...
            'acc_debit' => $request->amount,
            'acc_credit' => 0,
            'note' => $request->note,
            'currency_id' => $itemData['currency_id'],
        ]);
        return redirect('/accounts/receive_bond')->with('success', Lang::get('admin.added_successfully'));
    ...
    public function edit($id)
    {
        ...
        $receive_bond = Bond::where('branch_id', Auth::user()->branch_id)->find($id);
        $receive_bond_details = BondDetail::where('bond_id', $id)->get();

        return view($this->view_path . 'edit', array(
            'id' => $receive_bond->id,
            'receive_bond' => $receive_bond,
            'receive_bond_details' => $receive_bond_details,
    ...
    public function update(Request $request, $id)
    {
        ...
        $journal = Journal::find($receive_bond->journal_id);
        $journal->branch_id = Auth::user()->branch_id;
        $journal->operation_type_id = 3;
        $journal->journal_type_id = 1;
