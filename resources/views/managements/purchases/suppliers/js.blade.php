        ...
        mounted() { // الحاجة إلى الوصول للعناصر داخل الصفحة
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.edit-row')) { //هو العنصر الذي تم النقر عليه مباشرةً.
                    ...
                    this.editSuppliers(id, 0); // index يمكن تجاهله أو إضافته حسب الحاجة
                }
            });
        ...
        methods: {
            ...
            async searchSuppliers() {
                ...
                this.searchTimer = setTimeout(async () => {
                    try {
                        let url = "/purchases/suppliers/search";
            ...
            async loadSuppliers(page = 1) {
                try {
                    const res = await axios.get(`/purchases/suppliers/show?page=${page}&table_length=${this.perPage}`);
                    const data = res.data;
            ...
            resetButton() {
                ...
                this.dataUrl = "{{ route('suppliers.store') }}";
                this.dataType = "add";
            },
            ...
            async editSuppliers(id, index) {
                this.suppliersId = id;
                this.editRowIndex = index;

                this.buttonText = "{{ __('admin.edit') }}";
                this.buttonIcon = "ft-edit";
                this.dataUrl = `/purchases/suppliers/update/${id}`;
                this.dataType = "Update";

                try {
                    let url = `/purchases/suppliers/edit/${id}`;
            ...
            async saveSuppliers() {
                ...
                fd.append("suppliers_id", this.suppliersId);
            ...
            async deleteSuppliers(id, index) {
                const route = `/purchases/suppliers/destroy/${id}`;
