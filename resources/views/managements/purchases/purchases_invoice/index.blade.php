                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{__('admin.purchases_invoice')}}
                            </li>
                        </ol>
                    </div>
                </div>
            ...
            <div class="content-header-right col-md-6 col-12">
                <div class="media width-250 float-right">
                    ...
                    <div class="media-body media-right text-right">
                        <h3 class="m-0">@{{ currentCount }}</h3><span class="text-muted">{{__('admin.purchases_invoice')}}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-head">
                <div class="card-header">
                    <h4 class="card-title float-left">
                        <i class='bx bxs-file bx-tada' ></i> {{__('admin.purchases_invoice')}}
                    </h4>
                    <div class="float-right">
                        @can('create purchases_invoice')
                        <a class="btn btn-sm btn-blue-grey box-shadow-2 round btn-min-width pull-right white"
                           href="{{route('purchases_invoice.create')}}">
                            <i class="ft-plus white"></i>{{__('admin.create').' '.__('admin.purchases_invoice')}}
                        </a>
                            @endcan
                    </div>
                </div>
                        ...
                        <table width="200" id="table" class="table zero-configuration">
                            ...
                            <tr>
                                ...
                                <th>{{__('admin.note')}}</th>
                                                                                                                        ...
                                                                                                                        @canany(['update purchases_invoice', 'delete purchases_invoice'])
                            ...
                            <tr v-for="(row, index) in rows" :key="row.id || index">
                                ...
                                <td>
                                    <a :href="'/purchases/purchases_invoice/edit/' + row.id"><i
                                            class="la la-pencil-square success"></i></a>
                                    <a class="delete" :id="row.id" @click.prevent="deletePurchasesInvoice(row.id, index)"><i
                    ...
                    <nav v-if="lastPage > 1">
                        <ul class="pagination justify-content-end">
                            <li class="page-item" :class="{ disabled: currentPage === 1 }">
                                <a class="page-link" href="#" @click.prevent="loadPurchasesInvoice(currentPage - 1)">{{__('admin.Previous')}}</a>
                            </li>

                            <li class="page-item" v-for="page in lastPage" :key="page" :class="{ active: currentPage === page }">
                                <a class="page-link" href="#" @click.prevent="loadPurchasesInvoice(page)">@{{ page }}</a>
                            </li>

                            <li class="page-item" :class="{ disabled: currentPage === lastPage }">
                                <a class="page-link" href="#" @click.prevent="loadPurchasesInvoice(currentPage + 1)">{{__('admin.Next')}}</a>
                            </li>
                        </ul>
                    </nav>
    ...
    <script>
        ...
        createApp({
            ...
            mounted() {
                ...
                this.loadPurchasesInvoice();
            },
            methods: {
                async loadPurchasesInvoice(page = 1) {
                    try {
                        const res = await axios.get(`/purchases/purchases_invoice/show?page=${page}&table_length=${this.perPage}`);
                        const data = res.data;
                ...
                async deletePurchasesInvoice(id, index) {
                    const route = `/purchases/purchases_invoice/destroy/${id}`;
                ...
                async searchPurchasesInvoice() {
                    clearTimeout(this.searchTimer);
                    this.searchTimer = setTimeout(async () => {
                        try {
                            let url = "/purchases/purchases_invoice/search";
