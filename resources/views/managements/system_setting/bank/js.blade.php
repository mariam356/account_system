...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn-save').html('<i class="la la-check-square-o"></i> ' + "{{__('admin.add')}}")
            .attr("data_url", "{{route('bank.store')}}")
            .attr("data_type", "add");
    });
    ...
    $(document).on('click', '.edit-table-row', async function () {
        ...
        let url = "{{url('system_setting/bank/edit')}}" + '/' + bank_id;
        ...
        $('#btn-save').html('<i class="ft-edit"></i> ' + "{{__('admin.edit')}}")
            .attr("data_url", "{{url('system_setting/bank/update')}}" + '/' + bank_id)
    ...
    $('#form').on('submit', async function (event) {
        ...
        data_form.append('bank_id', bank_id);
        console.log(147);
        try {
            let data = await addOrUpdate(url, data_form, type, 'btn-save');
            if (data['status'] === 200) {
                let response_data = data.Bank;
    ...
    $(document).on('click', '.delete', async function () {
        ...
        let route = "{{url('system_setting/bank/destroy')}}" + "/" + id_row;
        try {
    ...
    $(document).on('keyup', '#search', async function () {
        ...
        const route = "{{ url('system_setting/bank/search') }}";
