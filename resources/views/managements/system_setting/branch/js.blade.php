        ...
        mounted() { // الحاجة إلى الوصول للعناصر داخل الصفحة
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.edit-row')) { //هو العنصر الذي تم النقر عليه مباشرةً.
                    ...
                    this.editBranch(id, 0); // index يمكن تجاهله أو إضافته حسب الحاجة
                }
            });
        ...
        methods: {
            ...
            async searchBranches() {
                ...
                this.searchTimer = setTimeout(async () => {
                    try {
                        let url = "/system_setting/branch/search";
            ...
            async loadBranches(page = 1) {
                try {
                    const res = await axios.get(`/system_setting/branch/show?page=${page}&table_length=${this.perPage}`);
                    const data = res.data;
            ...
            resetButton() {
                ...
                this.dataUrl = "{{ route('branch.store') }}";
                this.dataType = "add";
            },
            ...
            async editBranch(id, index) {
                this.branchId = id;
                this.editRowIndex = index;

                this.buttonText = "{{ __('admin.edit') }}";
                this.buttonIcon = "ft-edit";
                this.dataUrl = `/system_setting/branch/update/${id}`;
                this.dataType = "Update";

                try {
                    let url = `/system_setting/branch/edit/${id}`;
            ...
            async deleteBranch(id, index) {
                const route = `/system_setting/branch/destroy/${id}`;
