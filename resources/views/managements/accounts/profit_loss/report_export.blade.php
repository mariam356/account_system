...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('profit_loss.export', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('profit_loss.exportExcel', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
...
<table>
    ...
    <tbody>
        ...
        @foreach($data as $profit_loss)
            <tr>


                <td>{{$profit_loss->account->name ?? ''}}</td>
                <td>{{$profit_loss->total_debit ?? ''}}</td>
                <td>{{$profit_loss->total_credit ?? ''}}</td>
                <td>{{$profit_loss->balance ?? ''}}</td>
