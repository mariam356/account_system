...
<head>
    ...
    <title>{{ __('admin.accounting_guide') }}</title>
    <style>
        @font-face {
...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('account.export') }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('account.exportExcel') }}" class="btn btn-outline-primary">
...
<h1 style="text-align: center; border-bottom: 2px dashed #369fd9; padding-bottom: 10px; margin: 20px auto; width: 17%;">
    {{ __('admin.accounting_guide') }}
</h1>
<table>
    <thead>
    <tr>
    ...
    <tbody>
        ...
        @foreach($data as $accounting_guide)
            <tr>

                <td>{{ $accounting_guide->acc_code}}</td>
                <td>{{ $accounting_guide->account_id }}</td>
                <td>{{$accounting_guide->name}}</td>
                <td>{{$accounting_guide->acc_level}}</td>
                <td>{{$accounting_guide->acc_type->name ?? ''}}</td>
                <td>{{$accounting_guide->acc_report_type->name ?? ''}}</td>
                <td>{{$accounting_guide->acc_balance}}</td>
                <td>{{$accounting_guide->acc_debit}}</td>
                <td>{{$accounting_guide->acc_credit}}</td>

                <td>
                    @if($accounting_guide->status == 1)
                        @if($isPdf)
                            <img src="{{ public_path('logo//open-green.png') }}" width="18">
                        @else
                            <img src="{{ asset('logo//open-green.png') }}" width="18">
                        @endif
                    @else
                        @if($isPdf)
                            <img src="{{ public_path('logo/close-red.png') }}" width="18">
                        @else
                            <img src="{{ asset('logo/close-red.png') }}" width="18">
                        @endif
                    @endif
...
<div class="footer">
    ...
    <br>  <br>  <br>
    <p>{{ __('admin.generated_at') }}: {{ now()->format('Y-m-d H:i:s') }}</p>
</div>
