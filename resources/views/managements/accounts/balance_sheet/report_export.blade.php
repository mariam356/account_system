...
<head>
    ...
    <title>{{ __('admin.balance_sheet') }}</title>
    <style>
        @font-face {
...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('balance_sheet.export', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('balance_sheet.exportExcel', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
...
<table>
    ...
    <tbody>
        ...
        @foreach($data as $balance_sheet)
            <tr>


                <td>{{$balance_sheet->account->name ?? ''}}</td>
                <td>{{$balance_sheet->total_debit ?? ''}}</td>
                <td>{{$balance_sheet->total_credit ?? ''}}</td>
                <td>{{$balance_sheet->balance ?? ''}}</td>
