...
<head>
    ...
    <title>{{ __('admin.receive_bond') }}</title>
    <style>
        @font-face {
...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('receive_bond.export', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('receive_bond.exportExcel', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
...
<table style="width: 100%; border: none; margin-top: 50px;">
    <tr>
        <td style="width: 33%; text-align: {{ app()->getLocale() === 'ar' ? 'right' : 'left' }}; border: none;font-size: 1.0rem;">
            ...
            @foreach($data->unique('bond_id') as $receive_bond)
                <strong>{{ $receive_bond->bond->check_no  }}</strong>
...
<table>
    ...
    <tbody>
        ...
        @foreach($data as $receive_bond)
            <tr>

                <td>{{$receive_bond->id}}</td>
                <td>{{$receive_bond->account->acc_code ?? ''}}</td>
                <td>{{$receive_bond->account->name ?? ''}}</td>
                <td>{{$receive_bond->amount}}</td>

                <td>{{$receive_bond->currency->name ?? ''}}</td>
