    ...
    createApp({
        ...
        computed: {
            ...
            totalDebit() {
                return this.rows.reduce((sum, row) => {
                    const val = this.balanceAfterCurrency(row);
        ...
        methods: {
            openReport() {
                ...
                if (currentIds) {
                    const url = `/accounts/trial_balance/report_export?ids=${currentIds}`;
                    ...
                    window.open(`/accounts/trial_balance/report_export?ids=${ids}`, '_blank');
                }
            },
            balanceAfterCurrency(row) {
                const balance = Number(row.balance) || 0;
                return this.form.curr_val && this.form.curr_val != 0
                    ? balance / this.form.curr_val
                    : balance;
            },
            ...
            async search(page = 1) {
                ...
                try {
                    const res = await axios.get('/accounts/trial_balance/search', {
                        params: {
                            ...this.form,
                            page: page,
