@php($page_title = __('admin.exchange_bond'))
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header row mb-1">
            <div class="content-header-left col-md-6 col-12 mb-2">
                <h3 class="content-header-title">{{__('admin.exchange_bond')}}</h3>
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{__('admin.exchange_bond')}}
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    @can('report exchange_bond')
                        <div class="mb-2">
                            <button @click="printSelectedReports"
                                ...
                                <span
                                    style="font-weight: 600;">{{ __('admin.report') }} {{__('admin.exchange_bond')}}</span>
            ...
            <div class="card-head">
                <div class="card-header">
                    <h4 class="card-title float-left">
                        <i class='bx bxs-receipt bx-tada'></i> {{__('admin.exchange_bond')}}
                    </h4>
                    <div class="float-right">
                        @can('create exchange_bond')
                            <a class="btn btn-sm btn-blue-grey box-shadow-2 round btn-min-width pull-right white"
                               href="{{route('exchange_bond.create')}}">
                                <i class="ft-plus white"></i>{{__('admin.create').' '.__('admin.exchange_bond')}}
                            </a>
                        @endcan
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table width="200" id="table" class="table zero-configuration">
                            ...
                            <tr>
                                ...
                                @canany(['update exchange_bond', 'delete exchange_bond'])
                            ...
                            <tr v-for="(row, index) in rows" :key="row.id || index">
                                ...
                                <td>
                                    <a :href="'/accounts/exchange_bond/edit/' + row.id"><i
                                            class="la la-pencil-square success"></i></a>
                                    <a class="delete" :id="row.id" @click.prevent="deleteExchangeBond(row.id, index)"><i
                        ...
                        <ul class="pagination justify-content-end">
                            <li class="page-item" :class="{ disabled: currentPage === 1 }">
                                <a class="page-link" href="#"
                                   @click.prevent="loadExchangeBond(currentPage - 1)">{{__('admin.Previous')}}</a>
                            </li>

                            <li class="page-item" v-for="page in lastPage" :key="page"
                                :class="{ active: currentPage === page }">
                                <a class="page-link" href="#" @click.prevent="loadExchangeBond(page)">@{{ page }}</a>
                            </li>

                            <li class="page-item" :class="{ disabled: currentPage === lastPage }">
                                <a class="page-link" href="#"
                                   @click.prevent="loadExchangeBond(currentPage + 1)">{{__('admin.Next')}}</a>
                            </li>
                        </ul>
                    </nav>
        ...
        createApp({
            ...
            methods: {
                ...
                printSelectedReports() {
                    ...
                    window.open(`/accounts/exchange_bond/report?ids=${idsParam}`, '_blank');
                },
                async loadExchangeBond(page = 1) {
                    try {
                        const res = await axios.get(`/accounts/exchange_bond/show?page=${page}&table_length=${this.perPage}`);
                        ...
                        console.error("Error loading exchange_bond:", err);
                        // لا تفرغ المصفوفة هنا لكي لا تختفي البيانات القديمة في حال فشل الاتصال
                    }
                },

                async deleteExchangeBond(id, index) {
                    const route = `/accounts/exchange_bond/destroy/${id}`;
                ...
                async searchExchangeBond() {
                    clearTimeout(this.searchTimer);
                    this.searchTimer = setTimeout(async () => {
                        try {
                            let url = "/accounts/exchange_bond/search";
