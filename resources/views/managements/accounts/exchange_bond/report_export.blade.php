...
<head>
    ...
    <title>{{ __('admin.exchange_bond') }}</title>
    <style>
        @font-face {
...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('exchange_bond.export', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('exchange_bond.exportExcel', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
...
<h2 style="text-align: center; border-bottom: 2px dashed #369fd9; padding-bottom: 10px; margin: 20px auto; width: 17%;">
    {{ __('admin.exchange_bond') }}
</h2>
<table style="width: 100%; border: none; margin-top: 50px;">
    <tr>
        <td style="width: 33%; text-align: {{ app()->getLocale() === 'ar' ? 'right' : 'left' }}; border: none;font-size: 1.0rem;">
            <strong>{{__('admin.check_no')}} :</strong>
            @foreach($data->unique('bond_id') as $exchange_bond)
                <strong>{{ $exchange_bond->bond->check_no  }}</strong>
                {{-- إضافة فاصلة إذا لم يكن هذا هو العنصر الأخير --}}
                @if(!$loop->last) , @endif
            @endforeach
        </td>

        <td style="width: 33%; text-align: {{ app()->getLocale() === 'ar' ? 'left' : 'right' }}; border: none;font-size: 1.0rem;">
            <strong>{{__('admin.date')}} : {{ now()->format('Y-m-d H:i:s') }}</strong>
        </td>
    </tr>
</table>
<table>
    ...
    <tbody>
        ...
        @foreach($data as $exchange_bond)
            <tr>

                <td>{{$exchange_bond->id}}</td>
                <td>{{$exchange_bond->account->acc_code ?? ''}}</td>
                <td>{{$exchange_bond->account->name ?? ''}}</td>
                <td>{{$exchange_bond->amount}}</td>

                <td>{{$exchange_bond->currency->name ?? ''}}</td>
...
<div class="footer">
    <table style="width: 100%; border: none; margin-top: 50px; table-layout: fixed;">
        <tr>
            ...
            <td style="width: 25%; text-align: center; border: none; font-size: 1.0rem;">
                <strong>{{__('admin.fund')}} / {{__('admin.bank')}}</strong>
                <br><br>....................
            </td>
