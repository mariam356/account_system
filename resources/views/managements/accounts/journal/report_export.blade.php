...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('journal.export', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('journal.exportExcel', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
...
<table style="width: 100%; border: none; margin-top: 50px;">
    <tr>
        <td style="width: 33%; text-align: {{ app()->getLocale() === 'ar' ? 'right' : 'left' }}; border: none;font-size: 1.0rem;">
            ...
            @foreach($data->unique('journal_id') as $journal)
                <strong>{{ $journal->journal->journal_number  }}</strong>
    ...
    <tbody>
        ...
        @foreach($data as $journal)
            <tr>
                ...
                <td>{{$journal->account->acc_code ?? ''}}</td>
                <td>{{$journal->account->name ?? ''}}</td>
                <td>{{$journal->acc_debit}}</td>
                <td>{{$journal->acc_credit}}</td>
                <td>{{$journal->note}}</td>
                <td>{{$journal->currency->name ?? ''}}</td>
