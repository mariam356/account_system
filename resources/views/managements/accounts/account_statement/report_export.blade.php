...
<head>
    ...
    <title>{{ __('admin.account_statement') }}</title>
    <style>
        @font-face {
...
<div class="header">
    <div class="company-info">
        ...
        @endif
            @if(!$isPdf)
            <a style="float: left" href="{{ route('account_statement.export', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
                <img src="{{ asset('logo/Download PDF icon design.jfif') }}" width="60">
            </a>

                <a style="float: left" href="{{ route('account_statement.exportExcel', ['ids' => request('ids')])  }}" class="btn btn-outline-primary">
    ...
    <tbody>
        ...
        @foreach($data as $account_statement)
            <tr>
                ...
                <td>{{$account_statement->account->acc_code ?? ''}}</td>
                <td>{{$account_statement->account->name ?? ''}}</td>
                <td>{{$account_statement->acc_debit}}</td>
                <td>{{$account_statement->acc_credit}}</td>
                <td>{{$account_statement->note}}</td>
                <td>{{$account_statement->currency->name ?? ''}}</td>
