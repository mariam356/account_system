...
<script>
    ...
    createApp({
        ...
        mounted() { // الحاجة إلى الوصول للعناصر داخل الصفحة
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.edit-row')) { //هو العنصر الذي تم النقر عليه مباشرةً.
                    ...
                    this.editCustomer(id, 0); // index يمكن تجاهله أو إضافته حسب الحاجة
                }
            });
        ...
        methods: {
            ...
            async loadCustomer(page = 1) {
                try {
                    const res = await axios.get(`/sales/customer/show?page=${page}&table_length=${this.perPage}`);
                    const data = res.data;
            ...
            resetButton() {
                ...
                this.dataUrl = "{{ route('customer.store') }}";
                this.dataType = "add";
            },
            ...
            async editCustomer(id, index) {
                ...
                this.dataUrl = `/sales/customer/update/${id}`;
                this.dataType = "Update";

                try {
                    let url = `/sales/customer/edit/${id}`;
                    let data = await window.apiService.responseEditOrShowData2(url, this);


                    Object.assign(this.form, data);
                    this.form.debit_limit = data.debit_limit;
            ...
            async deleteCustomer(id, index) {
                const route = `/sales/customer/destroy/${id}`;
