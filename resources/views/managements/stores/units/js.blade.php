...
<script>
    ...
    createApp({
        ...
        mounted() { // الحاجة إلى الوصول للعناصر داخل الصفحة
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.edit-row')) { //هو العنصر الذي تم النقر عليه مباشرةً.
                    ...
                    this.editUnits(id, 0); // index يمكن تجاهله أو إضافته حسب الحاجة
                }
            });
        ...
        methods: {
            ...
            async loadUnits(page = 1) {
                try {
                    const res = await axios.get(`/stores/units/show?page=${page}&table_length=${this.perPage}`);
                    const data = res.data;
            ...
            resetButton() {
                ...
                this.dataUrl = "{{ route('units.store') }}";
                this.dataType = "add";
            },
            ...
            async editUnits(id, index) {
                this.unitsId = id;
                this.editRowIndex = index;

                this.buttonText = "{{ __('admin.edit') }}";
                this.buttonIcon = "ft-edit";
                this.dataUrl = `/stores/units/update/${id}`;
                this.dataType = "Update";

                try {
                    let url = `/stores/units/edit/${id}`;
            ...
            async saveUnits() {
                ...
                fd.append("units_id", this.unitsId);
            ...
            async deleteUnits(id, index) {
                const route = `/stores/units/destroy/${id}`;
