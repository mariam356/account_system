    ...
    createApp({
        ...
        data() {
            return {
                ...
                dataUrl: "{{ route('categories.store') }}",
                dataType: "add",
        ...
        mounted() { // الحاجة إلى الوصول للعناصر داخل الصفحة
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.edit-row')) { //هو العنصر الذي تم النقر عليه مباشرةً.
                    ...
                    this.editCategories(id, 0); // index يمكن تجاهله أو إضافته حسب الحاجة
                }
            });
        ...
        methods: {
            ...
            async searchCategories() {
                ...
                this.searchTimer = setTimeout(async () => {
                    try {
                        let url = "/stores/categories/search";
            ...
            async loadCategories(page = 1) {
                try {
                    const res = await axios.get(`/stores/categories/show?page=${page}&table_length=${this.perPage}`);
                    const data = res.data;
            ...
            resetButton() {
                ...
                this.dataUrl = "{{ route('categories.store') }}";
                this.dataType = "add";
            },
            ...
            async editCategories(id, index) {
                this.categoriesId = id;
                this.editRowIndex = index;

                this.buttonText = "{{ __('admin.edit') }}";
                this.buttonIcon = "ft-edit";
                this.dataUrl = `/stores/categories/update/${id}`;
                this.dataType = "Update";

                try {
                    let url = `/stores/categories/edit/${id}`;
            ...
            async saveCategories() {
                ...
                fd.append("categories_id", this.categoriesId);
            ...
            async deleteCategories(id, index) {
                const route = `/stores/categories/destroy/${id}`;
