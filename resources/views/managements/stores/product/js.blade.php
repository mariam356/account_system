    ...
    createApp({
        ...
        mounted() { // الحاجة إلى الوصول للعناصر داخل الصفحة
            ...
            document.addEventListener('click', (e) => {
                if (e.target.closest('.edit-row')) { //هو العنصر الذي تم النقر عليه مباشرةً.
                    ...
                    this.editProduct(id, 0); // index يمكن تجاهله أو إضافته حسب الحاجة
                }
            });
        ...
        methods: {
            ...
            async loadProducts(page = 1) {
                try {
                    const res = await axios.get(`/stores/product/show?page=${page}&table_length=${this.perPage}`);
                    const data = res.data;
            ...
            resetButton() {
                ...
                this.dataUrl = "{{ route('product.store') }}";
                this.dataType = "add";
            },
            ...
            async editProduct(id, index) {
                ...
                this.dataUrl = `/stores/product/update/${id}`;
                this.dataType = "Update";

                try {
                    let url = `/stores/product/edit/${id}`;
            ...
            async deleteProduct(id, index) {
                const route = `/stores/product/destroy/${id}`;
